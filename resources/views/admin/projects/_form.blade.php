<div class="space-y-4">
    ...
    <div>
        ...
        <select name="presentation_id" class="w-full border rounded px-3 py-2">
            @foreach($presentations as $p)
                <option value="{{ $p->id }}"
                    @selected(old('presentation_id', $project->presentation_id ?? '') == $p->id)>
                    {{ $p->titulo }} ({{ $p->periodo }})
                </option>
            @endforeach
        </select>
    ...
    <div>
        ...
        <input name="cliente_nombre"
               value="{{ old('cliente_nombre', $project->cliente_nombre ?? '') }}"
    ...
    <div>
        ...
        <input name="tecnologia"
               value="{{ old('tecnologia', $project->tecnologia ?? '') }}"
               class="w-full border rounded px-3 py-2">
    </div>
    ...
    <div class="grid grid-cols-2 gap-4">
        <div>
            ...
            <input name="estatus_label"
                   value="{{ old('estatus_label', $project->estatus_label ?? '') }}"
                   class="w-full border rounded px-3 py-2">
        </div>

        <div>
            <label class="font-semibold">Color Estatus</label>
            <input name="estatus_color"
                   value="{{ old('estatus_color', $project->estatus_color ?? '') }}"
                   class="w-full border rounded px-3 py-2">
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 gap-4">
        ...
        <div>
            ...
            <input name="horario"
                   value="{{ old('horario', $project->horario ?? '') }}"
                   class="w-full border rounded px-3 py-2">
        </div>
    </div>
